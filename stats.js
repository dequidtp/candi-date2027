...
async function loadStats() {
  ...
  let totalGames = 0;
  const perCandidate = {};
  CANDIDATES.forEach(c => { perCandidate[c.name] = { totalWins: 0, finalWins: 0, weightedScore: 0 }; });
  ...
  if (globalData) {
    ...
    (globalData.candidates || []).forEach(r => {
      if (perCandidate[r.candidate_name]) {
        ...
        perCandidate[r.candidate_name].weightedScore = r.total_weighted_score  || 0;
      }
    });
  } else {
    ...
    Object.entries(local.votes || {}).forEach(([name, v]) => {
      if (perCandidate[name]) {
        ...
        perCandidate[name].weightedScore = v.weightedScore || 0;
      }
    });
  }
  ...
  renderWeightedTable(perCandidate, totalGames);
}
...
// ── Table 3 — weighted score ───────────────────────────────────
function renderWeightedTable(perCandidate, totalGames) {
  const rows = CANDIDATES
    .map(c => ({ ...c, weightedScore: perCandidate[c.name].weightedScore }))
  ...
  const maxScore = rows[0].weightedScore || 1;
  const tbody    = document.getElementById('weightedBody');
  tbody.innerHTML = '';
 
  rows.forEach((c, i) => {
    const barPct = (c.weightedScore / maxScore) * 100;
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td class="stat-rank">${i + 1}</td>
      <td>
        <div class="stat-candidate">
          <div class="stat-photo-wrap" style="background:${c.color}">
            <img class="stat-photo hidden" id="wpi-${i}" alt="${c.name}" loading="lazy" />
            <div class="stat-avatar">${initials(c.name)}</div>
          </div>
          <div>
            <div class="stat-name">${c.name}</div>
      ...
      <td><span class="card-badge" style="${getBadgeStyle(c.color)}">${c.cat}</span></td>
      <td class="stat-num">${c.weightedScore}</td>
      <td class="stat-bar-wrap">
        <div class="stat-bar-track">
          <div class="stat-bar-fill" style="width:${barPct}%;background:linear-gradient(90deg,#a78bfa,#4db8ff)"></div>
        </div>
      </td>
    `;
    tbody.appendChild(tr);
    fetchWikipediaPhoto(c.name).then(url => {
      if (!url) return;
      const img = document.getElementById(`wpi-${i}`);
      if (!img) return;
      img.onload  = () => img.classList.remove('hidden');
      img.onerror = () => img.classList.add('hidden');
      img.src = url;
